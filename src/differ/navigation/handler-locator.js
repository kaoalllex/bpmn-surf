...
class HandlerLocator {
    ...
    static extractHandlerTopics(fileContent) {
        return [
            ...
            ...HandlerLocator.extractWrapToExternalTaskTopics(fileContent)
        ];
    }
    ...
    async #searchWrapToExternalTaskLocation(topic, ref) {
        ...
        // hit whose snippet shows the @WrapToExternalTask annotation (avoids
        // matching an unrelated class of the same name).
        const className = capitalizeFirstLetter(topic);
        return this.#searchClassLocation(className, ref, 'WrapToExternalTask');
    }

    // Locates the delegate class for a class:<Name> key — the same class-search
    // as #searchWrapToExternalTaskLocation but without an annotation to prefer.
    ...
    async #searchClassLocation(className, ref, preferAnnotation) {
        ...
        // (e.g. @WrapToExternalTask). Using .filter() instead of .find() is
