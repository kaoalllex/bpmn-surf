...
| `handler-locator.js` | `ExternalTaskHandlerLocator` — связь external-таски с кодом хендлера по топику: `findChangedHandlers` (скан изменённых `.kt` из MR changes API → `Map<topic, {filePath, diffType}>`, где `diffType` = `added`/`changed`/`removed`; удалённые файлы сканируются на target-ref, остальные — на mr-ref) и `resolveLocation` (GitLab blob-search → файл:строка). Два стиля объявления хендлера: `@ExternalTaskSubscription("<topic>")` (основной, топик задан явно) и `@WrapToExternalTask` (fallback — топик выводится из имени класса с первой буквой в нижнем регистре, аргументы аннотации опциональны). Точки расширения: языки (`.java`), делегаты (ключ — имя класса), глубокий анализ зависимостей |
