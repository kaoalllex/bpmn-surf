...
- `#searchClassLocation` — for class-name-based handler search (used by `@WrapToExternalTask`)
...
2. Fixed `#searchClassLocation` with the same pattern (for `@WrapToExternalTask` handlers)
...
- 1 regression test for `searchClassLocation` with multiple `@WrapToExternalTask` classes
