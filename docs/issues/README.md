...
Тип задачи определяется папкой и префиксом кода — отдельного поля `type` во frontmatter нет.
...
1. **Заведение.** Новая задача → новый файл в папке своего типа со следующим свободным номером в этой группе. Заполнить `Постановку` и `Контекст`; `status: open`; `Историю` оставить пустой.
...
3. **По завершении сессии ИИ над задачей** — добавить запись в `Историю работы` по шаблону выше и обновить `status`. Это требование закреплено в скиллах `feature`/`fix`/`refactor`.
4. **Связи** между задачами — по коду (`[FEAT-0003]`), не дублируя текст.
5. **Индекс не ведётся вручную.** Обзор бэклога — через поиск по frontmatter:
   ```
   grep -rn "^title:\|^status:\|^priority:" docs/issues
   ```
   Не открытые задачи отбираются по `status:`.
