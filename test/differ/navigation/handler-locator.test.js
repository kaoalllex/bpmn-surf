...
describe('HandlerLocator with real GitLab search results (BUG-0027)', () => {
    ...
    // handler files with @WrapToExternalTask annotation exist, the one with the
    // EXACT class name match is selected, not just the first.
    it('selects the correct handler when multiple @WrapToExternalTask classes exist (BUG-0027 regression)', () => {
        ...
        // Filter to ALL items with @WrapToExternalTask annotation (not just the first)
        const preferredItems = handlerItems.filter(i =>
            i.snippet && i.snippet.includes('@WrapToExternalTask')
        );

        // Both files should have the annotation
        assert.equal(preferredItems.length, 2,
            'Should have 2 handler files with @WrapToExternalTask annotation');
